...
def remove_cycle_edges_strategies(g,nodes_score_dict,score_name = "socialagony", nodetype = int):


	g1 = g.to_directed()
	# greedy
	e1 = scc_based_to_remove_cycle_edges_iterately(g1,nodes_score_dict)
	g2 = g.to_directed()
	# forward
	e2 = remove_cycle_edges_BF_iterately(g2, nodes_score_dict,is_Forward = True,score_name = score_name)
	# backward
	g3 = g.to_directed()
	e3 = remove_cycle_edges_BF_iterately(g3,nodes_score_dict,is_Forward = False,score_name = score_name)
	return e1,e2,e3

def remove_cycle_edges_by_voting(g,set_edges_list,nodetype = int):
	edges_score = get_edges_voting_scores(set_edges_list)
	e = remove_cycle_edges_heuristic(g,edges_score,nodetype = nodetype)
	return e 

def remove_cycle_edges_by_hierarchy(g,nodes_score_dict,score_name = "socialagony",nodetype = int):
	e1,e2,e3 = remove_cycle_edges_strategies(g,nodes_score_dict,score_name = score_name,nodetype = nodetype)
	e4 = remove_cycle_edges_by_voting(g,[set(e1),set(e2),set(e3)],nodetype = nodetype)
	return e1,e2,e3,e4

def computing_hierarchy(g,players_score_func_name, nodetype = int):
	import os.path
...
def breaking_cycles_by_hierarchy_performance(g, players_score_name, strategy, nodetype = int):
	
	#from measures import report_performance
	if players_score_name != "ensembling":
		players_score_dict  = computing_hierarchy(g, players_score_name,nodetype = nodetype)
		e1,e2,e3,e4 = remove_cycle_edges_by_hierarchy(g, players_score_dict,players_score_name,nodetype = nodetype)

		if strategy == 'scc_iteratively':
			return e1
		elif strategy == 'BF_iterately_forward':
			return e2
		elif strategy == 'BF_iterately_noforward':
			return e3
		...
		# commented because we don't use socialagony
		...
		players_score_dict  = computing_hierarchy(g, "trueskill",nodetype = nodetype)
		e5,e6,e7,e8 = remove_cycle_edges_by_hierarchy(g, players_score_dict,"trueskill",nodetype = nodetype)
		...
		e9 = remove_cycle_edges_by_voting(g,[set(e5),set(e6),set(e7)],nodetype = nodetype)
		#report_performance(gt_file,e9,"H_Voting")
		#write_pairs_to_file(e9,graph_file[:len(graph_file)-6] + "_removed_by_H-Voting.edges")


# strategy: scc_iteratively, BF_iterately_forward, BF_iterately_noforward, voting
def remove_edges_hierarchy(G, hierarchy="pagerank", strategy="scc_iteratively"):
	return breaking_cycles_by_hierarchy_performance(G, hierarchy, strategy)
